...
/*
 * This function runs in a separate client thread to handle communication with the server (unchanged from main submission)
 */
...
// To share with each server thread:
typedef struct {
	int epoll_fd;
	int listen_fd;
} server_thread_arg_t;

// Worker threads. Each thread will call epoll_wait on the same fd.
void* server_worker_thread(void *arg) {
	server_thread_arg_t *thread_arg = (server_thread_arg_t*) arg;
	int epoll_fd  = thread_arg->epoll_fd;
	int listen_fd = thread_arg->listen_fd;
...
void run_server(int num_server_threads) {
	...
	printf("Server listening on %s:%d with %d worker threads\n",
		   server_ip, server_port, num_server_threads);

	// Start multiple worker threads
	pthread_t *threads = malloc(sizeof(pthread_t) * num_server_threads);
	server_thread_arg_t thread_arg;
	thread_arg.epoll_fd  = epoll_fd;
	thread_arg.listen_fd = listen_fd;

	for (int i = 0; i < num_server_threads; i++) {
		pthread_create(&threads[i], NULL, server_worker_thread, &thread_arg);
	}
...
int main(int argc, char *argv[]) {
	if (argc > 1 && strcmp(argv[1], "server") == 0) {
		...
		// Allow inputting server threads as an option
		int num_server_threads = DEFAULT_SERVER_THREADS;
		if (argc > 4) {
			num_server_threads = atoi(argv[4]);
		}

		run_server(num_server_threads);
		...
		printf("Usage:\n");
		printf("  %s server [server_ip] [server_port] [server_threads]\n", argv[0]);
